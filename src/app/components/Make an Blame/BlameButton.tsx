...
export default function BlameButton({ editorRef, selectedFileData, session, title, comments }: BlameButtonProps) {

  return (
    <button
        className='py-2 px-2 mx-2 my-2 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg duration-150'
        onClick={() => submitPostFunc({ editorRef, selectedFileData, session, title, comments })}
    >
        Blame !
    </button>
  )
